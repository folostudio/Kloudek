  ...
  const handleFormSubmit = async (values: typeof initialValues) => {
    ...
    if (result) {
      try {
        ...
        const indexToUpdate = currentData.bedroom_furniture.findIndex(
          sofa => sofa.id === values.id
        );
  
        if (indexToUpdate !== -1) {
          // Update the object in the array with the new values
          currentData.bedroom_furniture[indexToUpdate] = updatedObject;
  
          // Update the document with the modified array
          await updateDoc(roomRef, {
            bedroom_furniture: currentData.bedroom_furniture
          });
          setOpen(false)
          setRender(true)
