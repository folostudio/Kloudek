...
const EditForm: FC<EditForm> = (props) => {
  ...
  const handleSpecificationChange = (index, event) => {
    const newValues = [...specification];
    newValues[index] = event.target.value;
    setSpecification(newValues);
  };
  ...
  const handleFormSubmit = async (values: typeof initialValues) => {
    const arr = [];
    arr.push(specification.map((value, index: number) => values[`specification_${index + 1}`]));
    const newArr = arr[0].filter(value => value !== undefined && value !== ''); // Lọc giá trị undefined và chuỗi trống
    const newSpecification = specification.filter(value => value !== ''); // Lọc chuỗi trống
    let updatedObject = {}
    if(areThereImage){
      updatedObject = {
        ...
        id: values.id,
        image: areThereImage ? imgUrl : values.image,
        ...
        specification: [...newArr, ...newSpecification]
      };
    }
    ...
    if (result) {
      try {
        ...
        const indexToUpdate = currentData.accent_chests_cabinnets.findIndex(
          sofa => sofa.id === values.id
        );
  
        if (indexToUpdate !== -1) {
          // Update the object in the array with the new values
          currentData.accent_chests_cabinnets[indexToUpdate] = updatedObject;
          // Update the document with the modified array
          await updateDoc(roomRef, {
            accent_chests_cabinnets: currentData.accent_chests_cabinnets
          });
          setOpen(false)
          setRender(true)
    ...
    // }
    areThereImage ? console.log(imgUrl) : console.log(values.image)
  };
        ...
        {({
          ...
          <form onSubmit={handleSubmit}>
            <Grid container spacing={3}>
              ...
              {/* {values.specification.map((value, index: number) => (
