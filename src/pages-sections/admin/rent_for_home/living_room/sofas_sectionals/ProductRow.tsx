...
const ProductRow: FC<ProductRowProps> = ({product, title, setRender}) => {
  ...
  // const convertedSpecification = specification !== null ? specification : [];
  // useEffect(() => {
  //   product.specification = convertedSpecification;
    ...
    const result = window.confirm("Bạn có chắc xóa sản phẩm");
      if(result){
        try {
          ...
          const updatedSofasSectionals = currentData.sofas_sectionals.filter(
            sofa => sofa.final_code !== id
          );

          // Update the document with the modified array
          await updateDoc(roomRef, {
            sofas_sectionals: updatedSofasSectionals
          });
      ...
      <StyledTableCell align="left">
        <FlexBox alignItems="center" gap={1.5}>
          ...
          <Box>
            ...
            <Small color="grey.600">{final_code}</Small>
          </Box>
        </FlexBox>
      </StyledTableCell>
